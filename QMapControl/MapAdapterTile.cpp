...
namespace qmapcontrol
{
              ...
              m_invert_y(invert_y)
    {
        ...
        QString dirzap1=dirzap+"/"+"razdan.ini";
        QSettings mSetting(dirzap1, QSettings::IniFormat);

        QVariant tmp;

        pathtiles=mSetting.value("MapData", tmp).toString();
    ...
    QUrl MapAdapterTile::tileQuery(const int& x, const int& y, const int& zoom_controller) const
    {
        ...
        /// @note QUrl converts % into %25, so we replace %25x, %25y and %25zoom instead.
          int Nx=0;
        int Ny=0;
        if(x>1024) Nx=(int)(x/1024);
        if(y>1024) Ny=(int)(y/1024);
     QString   host = "file:///";
     QString   url = pathtiles+"/z%1/%2/x%3/%4/y%5.png";
  //QString   url = "c:/SATI/z%1/%2/x%3/%4/y%5.jpg";
        const QString fetchURL = url.arg(QString::number(zoom_controller),
                                         QString::number(Nx),
                                         QString::number(x),
                                         QString::number(Ny),
                                         QString::number(y));
        return QUrl(host + fetchURL);

       // qDebug()<<"file="<<fetchURL<<pathtiles;
