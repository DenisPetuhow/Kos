...
namespace qmapcontrol
{
    ...
    PointWorldPx ProjectionSphericalMercator::toPointWorldPx(const PointWorldCoord& point_coord, const int& zoom) const
    {
        ...
        qreal y=point_coord.latitude()*M_PI/180.0;
        qreal e=0.081819790992114408;
        qreal lat=atanh(sin(y))-e*atanh(e*sin(y));
        const qreal y_px((1.0-lat/M_PI)/2.*(tilesY(zoom) * ImageManager::get().tileSizePx()));
    ...
    PointWorldCoord ProjectionSphericalMercator::toPointWorldCoord(const PointWorldPx& point_px, const int& zoom) const
    {
        ...
        qreal ypos=asin(tanh((1.0-point_px.y()*(2.0/(tilesY(zoom) * ImageManager::get().tileSizePx())))*M_PI));
        qreal n=0.003356551468879694;
        qreal k=0.00000657187271079536;
        qreal h=1.764564338702e-8;
        qreal m=5.328478445e-11;
        qreal g=ypos;
        qreal l=g+n*sin(2.0*g)+k*sin(4.0*g)+h*sin(6.0*g)+m*sin(8.0*g);
        const qreal latitude(l*180.0/M_PI);
