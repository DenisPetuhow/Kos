    ...
    void LayerMapAdapter::draw(QPainter& painter, const RectWorldPx& backbuffer_rect_px, const int& controller_zoom) const
    {
        ...
        if(isVisible(controller_zoom) && m_mapadapter != nullptr)
        {
            ...
            else
            {
              ...
              //  qreal dy=backbuffer_rect_px.topPx()-furthest_tile_top*256.0;
//qDebug()<<"backbuffer_rect_px.leftPx()="<<backbuffer_rect_px.leftPx()<<"backbuffer_rect_px.topPx()="<<backbuffer_rect_px.topPx()<<"z="<<controller_zoom<<"dy="<<dy;
