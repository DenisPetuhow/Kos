 ...
 */
ASDNoradSDP4::ASDNoradSDP4(ASDScTleInitData &tle) :
	ASDNoradEq(tle)
{
	m_sing = sin(m_tle.perigee);
	m_cosg = cos(m_tle.perigee);
	dp_savtsn = 0.0;
	dp_zmos = 0.0;
	dp_se2 = 0.0;
	dp_se3 = 0.0;
...
/*
 ...
 double a1 = pow( xke / tle.meanMotion, TWO_THRD );
 double temp = (1.5 * ck2 * (3.0 * sqr( cos(tle.inclination) ) - 1.0) /
 pow(1.0 - sqr(tle.eccentr), 1.5) );
 ...
 double _meanMotionRec = tle.meanMotion / (1.0 + delta0);
...
bool ASDNoradSDP4::DeepInit(double *eosq, double *sinio, double *cosio,
		double *betao, double *aodp, double *theta2, double *sing,
		double *cosg, double *betao2, double *xmdot, double *omgdot,
		double *xnodott)
{
	eqsq = *eosq;
	siniq = *sinio;
	...
	double jd = Jd(m_tle.epochYear, m_tle.epochDay);///!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!??????????

	dp_thgr = ToGmst(jd);
	//qDebug()()<<"jg="<<jd<<dp_thgr;
	double eq = m_tle.eccentr;
	double aqnv = 1.0 / ao;

	dp_xqncl = m_tle.inclination;

	double xmao = m_tle.meanAnomaly;
	double xpidot = omgdt + xnodot;
	double sinq = sin(m_tle.raan);
	double cosq = cos(m_tle.raan);

	dp_omegaq = m_tle.perigee;

	// Initialize lunar solar terms
	double day = jd - EPOCH_JAN0_12H_1900;//jd.fromJan1_12h_1900();
	//qDebug()()<<"day="<<day<<"dpi_day="<<dpi_day<<EPOCH_JAN0_12H_1900;
	if (day != dpi_day)
	{
		dpi_day = day;
	...
	for (int pass = 1; pass <= 2; pass++)
	{
		...
		z12 = -6.0 * (a1 * a6 + a3 * a5) + eqsq * (-24.0 * (x2 * x7 + x1 * x8)
				- 6.0 * (x3 * x6 + x4 * x5));
		z13 = -6.0 * a3 * a6 + eqsq * (-24.0 * x2 * x8 - 6.0 * x4 * x6);
		z21 = 6.0 * a2 * a5 + eqsq * (24.0 * x1 * x5 - 6.0 * x3 * x7);
		z22 = 6.0 * (a4 * a5 + a2 * a6) + eqsq * (24.0 * (x2 * x5 + x1 * x6)
				- 6.0 * (x4 * x7 + x3 * x8));
	...
	if ((m_xnodp >= 0.0052359877) || (m_xnodp <= 0.0034906585))
	{
		...
		else
		{
			...
			if (eq <= 0.65)
			{
				...
				g322 = -18.9068 + 109.7927 * eq - 214.6334 * eqsq + 146.5816
						* eoc;
				g410 = -41.122 + 242.694 * eq - 471.094 * eqsq + 313.953 * eoc;
				g422 = -146.407 + 841.880 * eq - 1629.014 * eqsq + 1083.435
						* eoc;
				g520 = -532.114 + 3017.977 * eq - 5740.0 * eqsq + 3708.276
						* eoc;
			}
			else
			{
				g211 = -72.099 + 331.819 * eq - 508.738 * eqsq + 266.724 * eoc;
				g310 = -346.844 + 1582.851 * eq - 2415.925 * eqsq + 1246.113
						* eoc;
				g322 = -342.585 + 1554.908 * eq - 2366.899 * eqsq + 1215.972
						* eoc;
				g410 = -1052.797 + 4758.686 * eq - 7193.992 * eqsq + 3651.957
						* eoc;
				g422 = -3581.69 + 16178.11 * eq - 24462.77 * eqsq + 12422.52
						* eoc;
			...
			if (eq < 0.7)
			{
				g533 = -919.2277 + 4988.61 * eq - 9064.77 * eqsq + 5542.21
						* eoc;
			...
			{
				g533 = -37995.78 + 161616.52 * eq - 229838.2 * eqsq + 109377.94
						* eoc;
			...
			double f522 = 9.84375 * siniq * (sini2 * (1.0 - 2.0 * cosiq - 5.0
					* cosq2) + 0.33333333 * (-2.0 + 4.0 * cosiq + 6.0 * cosq2));
			double f523 = siniq * (4.92187512 * sini2 * (-2.0 - 4.0 * cosiq
					+ 10.0 * cosq2) + 6.56250012 * (1.0 + 2.0 * cosiq - 3.0
					* cosq2));
			...
			dp_xlamo = xmao + m_tle.raan + m_tle.raan - dp_thgr - dp_thgr;
	...
	else
	{
		...
		double f311 = 0.9375 * siniq * siniq * (1.0 + 3 * cosiq) - 0.75 * (1.0
				+ cosiq);
		...
		dp_fasx4 = 2.8843198;
		dp_fasx6 = 0.37448087;
		dp_xlamo = xmao + +m_tle.raan + m_tle.perigee - dp_thgr;
...
bool ASDNoradSDP4::DeepCalcDotTerms(double *pxndot, double *pxnddt,
		double *pxldot)
{
	// Dot terms calculated
	if (dp_isynfl)
	{
		*pxndot = dp_del1 * sin(dp_xli - dp_fasx2) + dp_del2 * sin(2.0
				* (dp_xli - dp_fasx4)) + dp_del3 * sin(3.0
				* (dp_xli - dp_fasx6));
		*pxnddt = dp_del1 * cos(dp_xli - dp_fasx2) + 2.0 * dp_del2 * cos(2.0
				* (dp_xli - dp_fasx4)) + 3.0 * dp_del3 * cos(3.0 * (dp_xli
				- dp_fasx6));
	}
	else
	{
		...
		*pxndot = dp_d2201 * sin(x2omi + dp_xli - g22) + dp_d2211 * sin(dp_xli
				- g22) + dp_d3210 * sin(xomi + dp_xli - g32) + dp_d3222 * sin(
				-xomi + dp_xli - g32) + dp_d4410 * sin(x2omi + x2li - g44)
				+ dp_d4422 * sin(x2li - g44) + dp_d5220 * sin(xomi + dp_xli
				- g52) + dp_d5232 * sin(-xomi + dp_xli - g52) + dp_d5421 * sin(
				xomi + x2li - g54) + dp_d5433 * sin(-xomi + x2li - g54);

		*pxnddt = dp_d2201 * cos(x2omi + dp_xli - g22) + dp_d2211 * cos(dp_xli
				- g22) + dp_d3210 * cos(xomi + dp_xli - g32) + dp_d3222 * cos(
				-xomi + dp_xli - g32) + dp_d5220 * cos(xomi + dp_xli - g52)
				+ dp_d5232 * cos(-xomi + dp_xli - g52) + 2.0 * (dp_d4410 * cos(
				x2omi + x2li - g44) + dp_d4422 * cos(x2li - g44) + dp_d5421
				* cos(xomi + x2li - g54) + dp_d5433 * cos(-xomi + x2li - g54));
	}
...
void ASDNoradSDP4::DeepCalcIntegrator(double *pxndot, double *pxnddt,
...
bool ASDNoradSDP4::DeepSecular(double *xmdf, double *omgadf, double *xnode,
		double *emm, double *xincc, double *xnn, double *tsince)
{
	...
	_em = m_tle.eccentr + dp_sse * t;
	xinc = m_tle.inclination + dp_ssi * t;
	...
	if (dp_iresfl)
	{
		while (!fDone)
		{
			if ((dp_atime == 0.0) || ((t >= 0.0) && (dp_atime < 0.0)) || ((t
					< 0.0) && (dp_atime >= 0.0)))
			{
				if (t < 0)
					delt = dp_stepn;
...
bool ASDNoradSDP4::DeepPeriodics(double *e, double *xincc, double *omgadf,
...
QVector<double> ASDNoradSDP4::getPosition(double tsince)
{
	...
	//qDebug()()<<"sdp4 pos init1 =="<<m_eosq<<m_sinio<<m_cosio<<m_betao<<m_aodp<<m_theta2;
	//qDebug()()<<"sdp4 pos init 2 =="<<m_sing<<m_cosg<<m_betao2<<m_xmdot<<m_omgdot<<m_xnodot;
	...
	double em;
	double xinc;
	//qDebug()()<<"tle omega="<<omgadf*180./M_PI<<"xn="<<xn;
	DeepSecular(&xmdf, &omgadf, &xnode, &em, &xinc, &xn, &tsince);
	//qDebug()()<<"sdp4 DeepSecular="<<omgadf*180./M_PI<<xinc*180./M_PI<<xn<<tsince;
	...
	pos = finalPosition(xinc, omgadf, e, a, xl, xnode, xn, tsince);
	return pos;
}
